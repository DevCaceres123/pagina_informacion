...
class AdministrativoImport implements ToModel, WithHeadingRow, WithValidation
{
    use Importable;
    ...
    public function model(array $row)
    {
        ...
        $this->documentosImportados[] = "{$sedeId}-{$row['documento']}-{$row['cargo']}";
    ...
    {
        EstadisticaAdministrativo::upsert(
            $this->dataToInsert,
            ['sede_id', 'n_documento', 'cargo'],
            ['nombre_completo', 'n_documento','genero', 'servicio', 'profesion','cargo','updated_at']
        );
    ...
    public function rules(): array
    {
        return [
            ...
            'profesion' => 'required|max:100|min:1',
            'cargo' => 'required|max:100|min:3',
            'servicio' => 'required|in:contrato,planta,linea',
        ];
    }
    ...
    private function desactivarFaltantes()
    {
        ...
        if ($this->gestionArchivo) {
            ...
            $existenRegistrosPrevios = EstadisticaAdministrativo::where('gestion', $this->gestionArchivo)->exists();

            if ($existenRegistrosPrevios) {
                EstadisticaAdministrativo::where('gestion', $this->gestionArchivo)
                 ->whereNotIn(DB::raw("CONCAT(sede_id,'-',n_documento,'-',cargo)"), $this->documentosImportados)
